...
template <typename T> void log(std::string output, T i) {
  if (LOG) {
    std::ofstream out;
    out.open("log.txt", std::ios::app);
...
bool PREDICTOR::GetPrediction(UINT32 PC) {
  ...
  for (UINT32 i = pred.table + 1; i < NUM_TAGE_TABLES;
       i++) { // check for tag hits on lower tables
    if (tagTables[i][tageIndex[i]].tag == tageTag[i]) { // tag hit
      pred.altTable = i;
  ...
  if (pred.table < NUM_TAGE_TABLES) {       // if we haven't missed a table
    if (pred.altTable == NUM_TAGE_TABLES) { // if altPred missed a table
      pred.altPred =
          (bimodal[bimodalIndex].pred > BIMODAL_PRED_MAX / 2); // use bimodal
    } else { // if altpred hit a table
      if (tagTables[pred.altTable][pred.altIndex].pred >=
          TAGE_PRED_MAX / 2) // use bimodal prediction
        pred.altPred = TAKEN;
      else
        pred.altPred = NOT_TAKEN;
    }
    if ((tagTables[pred.table][pred.index].pred !=
         WEAKLY_NOT_TAKEN) || // if pred is not weak,
...
void PREDICTOR::UpdatePredictor(UINT32 PC, bool resolveDir, bool predDir,
  ...
  if (pred.table < NUM_TAGE_TABLES) {                 // if we hit an entry
    if ((tagTables[pred.table][pred.index].u == 0) && // if entry is not useful
        ((tagTables[pred.table][pred.index].pred ==
          WEAKLY_NOT_TAKEN) || // and weakly predicted
         (tagTables[pred.table][pred.index].pred == WEAKLY_TAKEN))) {
  ...
  if ((!newInTable) ||
      (newInTable &&
       (pred.pred != resolveDir))) { // if table's not new, or pred is wrong
    if (((predDir != resolveDir) &
         (pred.table > 0))) { // if pred is wrong and there was a tag miss
      bool alloc = false;
  ...
  if (clock == (1 << CLOCK_MAX)) { // currently 256k as in paper
    ...
    for (UINT32 i = 0; i < NUM_TAGE_TABLES; i++) { // for all tags
      for (UINT32 j = 0; j < tageTableSize; j++) {
        tagTables[i][j].u &= (clockState + 1); // if clockstate = 0, reset lower
                                               // bit else reset upper bit
      }
    }
  }
  // update the GHR
...
UINT32 PREDICTOR::getIndex(UINT32 PC, int table, UINT32 tagSize,
                           UINT32 phrOffset) {
  UINT32 index = PC ^ (PC >> tagSize) ^ csrIndex[table].val ^ PHR ^
                 (PHR & ((1 << phrOffset) - 1));
