...
void print_stats(long long int dmiss, long long int tmiss) {
  printf("%lld instructions; %0.3f IPB; %0.3f direction MPKI; %0.3f indirect "
         "MPKI\n",
         trace_instructions, instructions_per_branch,
         1000.0 * (dmiss / (double)trace_instructions),
         1000.0 * (tmiss / (double)trace_instructions));
  fflush(stdout);
}
...
int main(int argc, char *argv[]) {
  ...
  long long int last_instructions = 0,
