...
struct remember {
  ...
  bool equal(remember *r, bool ignore_target) {
    return r->code == code && r->taken == taken && r->address == address &&
...
void init_ras(void) { ras_top = RAS_SIZE; }
...
void update_remember(remember &me, remember *r, bool correct, int index) {
  if (correct) {
    ...
    for (int i = 1; i < ASSOC; i++)
      if (r[i].lru_time < r[lru].lru_time)
        lru = i;
    r[lru] = me;
    r[lru].lru_time = now++;
  }
...
trace *read_trace1(void) {
  ...
  if (correct) {
    ...
    if (r.code == 0x70) {
      ...
      if (popd != t.target && popd != t.target - 2 && popd != t.target + 3)
        init_ras();
    }
...
void end_trace(void) { fclose(tracefp); }
